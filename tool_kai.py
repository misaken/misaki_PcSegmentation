...
if __name__ == "__main__":
    ...
    for i in range(0, n_frame+1-skip, skip):
        ...
        Y = pc[i+skip]
        n_clusters = 5
        points_for_genT = np.empty([0, n_sample, 3], dtype=int) # inlierの個数がしきい値以上の剛体変換を求めるときに使った点
        ...
        for j in range(n_clusters): #クラスター数だけ繰り返す。
            ...
            for k in range(n_iter):
                ...
                if n_inlier > d:
                    ...
                    print(f"T=\n{np.round(T, 4)}")
                    points_for_genT = np.concatenate([points_for_genT, X[random_idx].reshape([1, n_sample, 3])])
                    label += 1
                    break
            else:
                print("can not fix")
