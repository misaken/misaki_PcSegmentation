...
if __name__ == "__main__":
    ...
    # for i in range(n_frame-1):
        ...
        for j in range(n_clusters): #クラスター数だけ繰り返す。
            ...
            t_s = time.time()
            for k in range(n_iter // batch_size):
                # クラス0の点からサンプリング
                # random_idx = np.random.choice(zero_idx, n_sample, replace=False)
                random_idx = np.array([np.random.choice(zero_idx, n_sample, replace=False) for _ in range(batch_size)])
                T = svd2T(X[random_idx], Y[random_idx]) # T: (batch_size, 4, 4)
                #print(T)
                Y_pred = rigid_transform(T, X) #Y_pred : (batch_size, num_data, 3)
                pdb.set_trace()
